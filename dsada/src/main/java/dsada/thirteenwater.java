...
class Pocker
{
	...
	private void getzuhe(Vector<pai> vv,int x,int start)
	{
		...
		else
		{
			Enumeration<pai> element=vv.elements();
				...
				//到这里fen1>fen2>fen3
				if(fen1*1.0+fen2*0.7+fen3*1.3>allscore)
				{
					allscore=fen1*1.0+fen2*0.7+fen3*1.3;
	...
	private void getzuhe2(Vector<pai> vv,int x,int start)
	{
		if(x>8)
		{
				for(int i=start;i<=vv.size();i++)
		...
		else
		{
			Enumeration<pai> element=vv.elements();
				...
				//到这里fen1>fen2>fen3
				if(fen1*0.5+fen2*0.1+fen3*10>allscore)
				{
					allscore=fen1+fen2*0.5+fen3*2;
					getv1.removeAllElements();
					getv2.removeAllElements();
					getv3.removeAllElements();
					element=v1.elements();
	...
	private int judgehulu(Vector<pai> vv)
	{
		...
		{
			score=score+santiao*1000*10/15;
			return score;
		}
		else return 0;
	}
	private double judgetonghua(Vector<pai> vv)
	{
		double score=70000;int[]num= {0,0,0,0,0,0};int i=5;double sc=0;
		char lastcolor=0;int j=0;
		Enumeration<pai> element=vv.elements();
		while(element.hasMoreElements())
		{
			if(j==0)
			...
			{
				pai a=element.nextElement();
				num[i]=change(a.getvalue());i--;
				if(a.getcolor()!=lastcolor) return 0;
			}
		}
		for(i=1;i<=5;i++)
		{
			if(num[i]!=0) sc=(sc+num[i])*100;
		}
		if(vv.size()==3) sc=sc*10000;
		for(i=1;i<=4;i++) sc=sc/100;
		return score+sc*10/15;
	}
	...
	private int judgesantiao(Vector<pai> vv)
	{
		...
		if(santiao!=0)
			return score+santiao*1000*10/15;
		else return 0;
	}
	private int judgeliandui(Vector<pai> vv)
	{
		...
		if(count==2&&dui2-dui1==1) return score+dui2*1000*10/15;
		else return 0;
	}
	...
	private int judgedandui(Vector<pai> vv)
	{
		...
		if(count==1) return score+dui*10/15;
			else return 0;
	}
	private double judgesan(Vector<pai> vv)
	{
		...
		if(count==3) score=score*10000;
		for(int i=1;i<=4;i++) score=score/100;
		score=10000+score*10/15;
		return score;
	}
	...
	{
		Enumeration<pai> element=v.elements();
		int i=1;String card=new String();
		while(element.hasMoreElements())
		{
			card=card+element.nextElement().getcard();
			if(i==3||i==8) card=card+"\",\"";
			else if(i!=13) card=card+" ";
			i++;
		}
		return card;
	}
